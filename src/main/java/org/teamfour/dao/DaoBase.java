...
public abstract class DaoBase extends StaticLogBase {
    ...
    public Connection getConnection() {
        try {
            ...
            Connection conn =  DriverManager.getConnection(SystemFiles.REGISTRY_DB_PATH + "votingData.db");
            return conn;
    ...
    public void closeIfPresent(AutoCloseable o) {
        if (o != null) {
            try {
                ...
                log.warn("ERROR CLOSING RESOURCE");
            }
        }
    }
    ...
    private int delete(String sql) {
        ...
        try {
            ...
            log.error("SQL EXCEPTION ON SAVE");
    ...
    private <T> String findEntityQuery(Class<T> c, long primaryKey) {
        ...
        if (tableName == null ) {
            log.error("CLASS IS NOT A TABLE ENTITY");
